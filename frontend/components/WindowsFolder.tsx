...
export default function WindowsFolder({ folder, projects, isRoot = false, pathPrefix = '' }: Props) {
  ...
  const totalItems = children.length + folderProjects.length
        ...
        <div className="ml-6 mt-1 space-y-1 animate-fadeIn">
          ...
          {folderProjects.length > 0 ? (
            folderProjects.map((project) => (
              <Link
                key={project.id}
                href={`/projects/${folderPath}/${project.slug}`}
